...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('tutor-login-form').addEventListener('submit', function(event) {
        ...
        const username = document.querySelector('input[name="log"]').value;
        const password = document.querySelector('input[name="pwd"]').value;

        fetch('https://fatsus.com.br/wp-json/jwt-auth/v1/token', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            ...
            if (data.token) {
                ...
                window.location.href = 'https://fatsus.com.br/painel/';
            } else {
                document.querySelector('.tutor-login-error').innerText = 'Erro no login';
            }
        })
